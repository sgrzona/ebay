...
describe 'Part 1 AuctionBidController Specs', :part_1_specs => true do
  let!(:user) { create(:user) }
  before do 
    create(:auction)
  end

  describe AuctionBidController do
    describe 'GET new' do
    ...
    describe 'POST create' do
      ...
      let!(:auctionbid) { build(:low_auctionbid) }
      ...
      context 'with a high enough amount' do
        before(:each) do
          auction.auctionbids.create(:amount => auctionbid.amount)
          post :create, { auction_id: auction.id, auctionbid: {
                                                amount: auctionbid.amount + 1000
                                               }
                        }
        end
        ...
        it 'creates a new bid' do
          ...
          expect(AuctionBid.all.count).to eq(2)
        end
      ...
      context 'with a bid lower than the current high bid' do
        before(:each) do
          auction.auctionbids.create(:amount => auctionbid.amount)
          post :create, { auction_id: auction.id, auctionbid: {
                                                amount: auctionbid.amount - 10
                                               }                         
                        }
        end

        it 'doesn\'t create a new bid' do
          expect(AuctionBid.all.count).to eq(1)
        end
    ...
    describe 'POST create' do
      ...
      context 'with a logged in user' do
        context 'on another user\'s auction' do
          before do
            use_user_id(auction.seller_id + 1)
            post :create, { auction_id: auction.id, auctionbid: { amount: 1000 } }
          end
        ...
        context 'on the current user\'s auction' do
          before do
            ...
            post :create, { auctionbid_id: auction.id, auctionbid: { amount: 1000 } }
          end
